...
class Dataloaders:
    ...
    def prepare_dataloaders(self):
        ...
        data_size = len(self.images)
        data_ids = np.random.shuffle(np.range(data_size))
        train_ids, test_ids = data_ids[:int(data_ids * self.train_test_ratio)], data_ids[int(data_ids * self.train_test_ratio):]

        self.train = torch.utils.data.DataLoader(
            MyDataset(images, labels, train_ids),
            batch_size = self.batch_size,
            shuffle=True,
            num_workers=2
        )
        ...
        self.test = torch.utils.data.DataLoader(
            MyDataset(images, labels, test_ids),
            batch_size = self.batch_size,
            shuffle=False,
            num_workers=2
        )
